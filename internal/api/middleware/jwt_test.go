...
func TestJWTAuthMiddleware_ValidToken(t *testing.T) {
	...
	token, err := jwtService.GenerateToken("user123", "test@example.com", 1*time.Hour)
	if err != nil {
...
func TestJWTAuthMiddleware_ExpiredToken(t *testing.T) {
	...
	jwtService := security.NewJWTService("test-secret-key-min-32-chars")
	token, err := jwtService.GenerateToken("user123", "test@example.com", -1*time.Hour) // Expired
	if err != nil {
		t.Fatalf("Failed to generate token: %v", err)
	}
