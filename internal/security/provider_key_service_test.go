...
func TestNewProviderKeyService(t *testing.T) {
	...
	service, err := NewProviderKeyService(repo, encryptionKey)
	if err != nil {
	...
	if len(service.encryptionKey) != 32 {
		t.Errorf("Expected encryption key length 32, got %d", len(service.encryptionKey))
	}
}
...
func TestProviderKeyService_AddProviderKey(t *testing.T) {
	repo := newMockProviderKeyRepository()
	encryptionKey := "test-encryption-key-min-32-chars-long"
	service, _ := NewProviderKeyService(repo, encryptionKey)
