...
| Tunnel | *(empty or .)* | `Dockerfile.tunnel` | 8080 + **20000-30000** | \*.uniroute.co |
...
- **Tunnel:** Use `Dockerfile.tunnel`; set `TUNNEL_BASE_DOMAIN`, `WEBSITE_URL`, `DATABASE_URL`, `JWT_SECRET`, `API_KEY_SECRET`. For wildcard SSL use DNS challenge (e.g. Cloudflare) in Coolify/Traefik.
  - **TCP/TLS/UDP tunnels:** Publish the TCP port range **20000-30000** in addition to the main port (8080 or 8081). In Coolify, add a **Port** mapping: container port range `20000-30000` → same host range (e.g. `20000-30000:20000-30000`). Without this, `uniroute tcp 3306` will connect but connections to the public URL (e.g. `subdomain.tunnel.uniroute.co:20000`) will fail with "Connection refused". Also open 20000-30000 on the host firewall (e.g. `ufw allow 20000:30000/tcp`).
