...
- **Tunnel:** Use `Dockerfile.tunnel`; set `TUNNEL_BASE_DOMAIN`, `WEBSITE_URL`, `DATABASE_URL`, `JWT_SECRET`, `API_KEY_SECRET`. For wildcard SSL use DNS challenge (e.g. Cloudflare) in Coolify/Traefik.
  - **TCP/TLS/UDP tunnels:** Publish a TCP port range in addition to the main port (8080 or 8081). Recommended: **20000-20100** (101 ports) with `TUNNEL_TCP_PORT_RANGE=101` for faster Docker/Coolify deploys; or **20000-30000** (10000 ports) for many concurrent tunnels. In Coolify, add a **Port** mapping: e.g. `20000-20100:20000-20100`. Without this, `uniroute tcp 3306` will connect but connections to the public URL will fail with "Connection refused". Open the same range on the host firewall (e.g. `ufw allow 20000:20100/tcp`).
