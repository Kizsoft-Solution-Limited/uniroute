...
- **Tunnel:** Use `Dockerfile.tunnel`; set `TUNNEL_BASE_DOMAIN`, `WEBSITE_URL`, `DATABASE_URL`, `JWT_SECRET`, `API_KEY_SECRET`. For wildcard SSL use DNS challenge (e.g. Cloudflare) in Coolify/Traefik.
  ...
  - **Health check:** `GET /health` returns `200` with `{"status":"ok","tunnels":N,"connected":M}` when healthy. If there are tunnel entries but no active client connection (`tunnels` > 0 and `connected` == 0), the server returns **503**. Use `/health` as the Coolify health check URL so that when the tunnel server is in this stale state, Coolify restarts the container and traffic works again after clients reconnect.
