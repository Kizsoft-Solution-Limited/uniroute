<template>
  <div class="space-y-6">
    <!-- Header -->
    <div>
      <h1 class="text-3xl font-bold text-white">Custom Routing Rules</h1>
      <p class="text-slate-400 mt-1">
        Define global custom routing rules that apply to all users by default
      </p>
    </div>

    <!-- Admin Warning Banner -->
    <div class="bg-amber-500/10 border-2 border-amber-500/30 rounded-lg p-4 shadow-lg">
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1">
          <h3 class="font-bold text-amber-400 mb-1 text-lg">⚠️ Global Custom Routing Rules</h3>
          <p class="text-sm text-amber-300/90 mb-2">
            These are <strong class="text-amber-200">global rules</strong> that apply to all users when the default routing strategy is set to "Custom". Individual users can override these by setting their own custom routing strategy and defining their own custom rules.
          </p>
          <p class="text-xs text-amber-300/70">
            Rules are evaluated in priority order (higher priority first). If no rules match, the system falls back to model-based routing.
          </p>
        </div>
      </div>
    </div>
    ...
    <Card>
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          <h2 class="text-xl font-semibold text-white">Global Custom Rules</h2>
        ...
        <div v-else-if="rules.length === 0" class="text-center py-8 text-slate-400">
          <p>No global custom rules defined yet.</p>
          <p class="text-sm mt-2">Click "Add Rule" to create your first global custom routing rule.</p>
        </div>
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1 space-y-2">
                <div class="flex items-center gap-3">
                  <h3 class="font-semibold text-white">{{ rule.name || `Rule ${index + 1}` }}</h3>
                  <span
    ...
    <!-- Rule Editor Modal (same as user version) -->
    <div
      ...
      <Card class="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div class="space-y-4">
          <h3 class="text-xl font-bold text-white">
            {{ editingRule === -1 ? 'Add Global Rule' : 'Edit Global Rule' }}
          </h3>
...
const loadRules = async () => {
  ...
  try {
    const response = await routingApi.getAdminCustomRules()
    rules.value = response.rules || []
  } catch (err: any) {
    const appError = ErrorHandler.handleApiError(err)
    showToast(`Failed to load rules: ${appError.message}`, 'error')
    ErrorHandler.logError(err, 'AdminCustomRules')
  } finally {
    loading.value = false
  }
}
...
const saveRules = async () => {
  ...
  try {
    const response = await routingApi.setAdminCustomRules({ rules: rules.value })
    updateResult.value = {
      success: true,
      message: response.message || 'Global custom rules saved successfully'
    }
    showToast('Global custom rules saved successfully', 'success')
    await loadRules()
  } catch (err: any) {
    const appError = ErrorHandler.handleApiError(err)
    updateResult.value = {
...
const deleteRule = (index: number) => {
  if (confirm('Are you sure you want to delete this global rule?')) {
